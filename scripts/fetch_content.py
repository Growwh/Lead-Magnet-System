...
def fetch_youtube(url: str) -> str:
    ...
    print(f"[fetch] Fetching transcript for video: {video_id}", file=sys.stderr)
    ...
    try:
        ...
        if transcript is None:
            sys.exit(f"No transcript available for video: {video_id}")

        entries = transcript.fetch()

        # Clean up: remove filler, add paragraph breaks at natural pauses
        cleaned = _preprocess_transcript(entries)

        return cleaned

    except Exception as e:
        sys.exit(f"Failed to fetch YouTube transcript: {e}")


def _preprocess_transcript(entries: list) -> str:
    """Clean up transcript: remove filler words, add paragraph breaks at topic shifts."""
    # Combine entries into chunks based on timing gaps
    paragraphs = []
    current_paragraph = []
    prev_end = 0
    ...
    for entry in entries:
        ...
        # If there's a gap of 2+ seconds, start a new paragraph (topic shift)
        gap = start - prev_end
...
def fetch(input_str: str, no_cache: bool = False) -> str:
    ...
    # YouTube
    if "youtube.com/watch" in s or "youtu.be/" in s or "youtube.com/shorts/" in s:
