...
Generates a high-quality branded PDF using Playwright (Chromium).
Produces Notion-quality rendering with full CSS support, Google Fonts,
and proper page-break handling.

After generating, applies running footer via PyMuPDF.
...
def load_logo_b64() -> str:
    ...
    for name in ("logo.png", "logo-converted.png"):
        p = ASSETS_DIR / name
        if p.exists():
...
def preprocess_markdown(text: str, images_dir: str | None = None) -> str:
    """Convert custom syntax to HTML. Returns raw HTML snippets
...
# -- Postprocess: footer via PyMuPDF --

def postprocess_pdf(pdf_path: str, brand_name: str = "Your Brand"):
    try:
        import fitz
    except ImportError:
    ...
    try:
        for page_num, page in enumerate(doc):
            ...
            page.insert_text(fitz.Point(left_margin, footer_y), f"{brand_name}", fontsize=7, color=grey)
